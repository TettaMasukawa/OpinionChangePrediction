...
onc_word_list = [
    "これからも支持する", "これからも支持します", "これからも賛同する", "これからも賛同します", "これからも意見変えない", "これからも意見変えません",
    "これからも考え変えない", "これからも考え変えません", "これからも支持変えない", "これからも支持変えません", "これからも立場変えない", "これからも立場変えません",
    "これからも見方変えない", "これからも見方変えません", "これからも見解変えない", "これからも見解変えません", "考え改めない", "考え改めません",
    "考えを変えない", "考えを変えません", "考えを改めない", "考えを改めません", "支持続ける", "支持続けます", "これからも意見変えない", "これからも意見変えません",
    "これからも考え変えない", "これからも考え変えません", "これからも支持変えない", "これからも支持変えません", "これからも立場変えない", "これからも立場変えません",
    "これからも見方変えない", "これからも見方変えません", "これからも見解変えない", "これからも見解変えません", "考え改めない", "考え改めません",
    "ずっと支持する", "ずっと支持します", "ずっと賛同する", "ずっと賛同します", "これからも意見一緒", "これからも意見同じ", "これからも考え一緒",
    "これからも考え同じ", "これからも支持一緒", "これからも支持同じ", "これからも立場一緒", "これからも立場同じ", "これからも見方一緒", "支持する"
    "これからも支持する", "これからも支持します", "これからも賛同する", "これからも賛同します", "これからも意見変えない", "これからも意見変えません",
    "これからも熱烈", "これからも続く", "これからも支持", "これからも賛同", "これからも賛成", "これからも同意", "支持したい", "支持します", "支持する",
    "賛同したい", "賛同します", "賛同する", "賛成したい", "賛成します", "賛成する", "同意したい", "同意します", "同意する", "これからもする", "これからも続ける",
    "これからも"
]
...
users = sorted(glob("/home/tmasukawa/tweet-analysis/get_and_format/twitter-scraper/data/opinion_non_changers/users/*.csv"))

cnt = 0

opinion_non_changer_features = []

onc_post_embeddings = []
onc_repost_embeddings_dict = []

for user in tqdm(users):
    user_handle = user.split("/")[-1].split(".")[0]
    ...
    for onc_word in onc_word_list:
        if onc_word in flag_df["Content"]:
            word_match = True
            break
    ...
    for i in range(len(posts_list)):
        ...
        opinion_non_changer_features.append(features)

    cnt += 1

# Set the path to save the features
with open("data/features/onc_features_tweets_random-BERT.json", "w") as f:
    json.dump(opinion_non_changer_features, f, indent=4, ensure_ascii=False)
